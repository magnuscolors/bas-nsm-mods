...
class report_rappel(report_sxw.rml_parse):
    ...
    def translate(self, name):
        lang = self.pool.get('res.users').browse(self.cr, self.uid, self.uid).lang
    ...
    def date_month(self, date_split):
        """docstring for date Month"""
        res = ''
        temp = time.strftime('%Y-%m-%d').strip()
        if temp not in ['False' or False]:
            date_format = datetime.datetime.strptime(str(temp), '%Y-%m-%d')
            res = str(datetime.datetime.strftime(date_format, '%B'))
        return self.translate(res)

report_sxw.report_sxw('report.account_followup.followup.print.nsm',
        'account_followup.stat.by.partner', 'addons/nsm_followup_layout/report/account_followup_print.rml',
        parser=report_rappel, header=False)
